/* ============ 铸魂塔增益图标动态加载（启动脚本） ============ */
// 功能：每次加载项目页面时，从 s.61.com 实时抓取铸魂塔增益图标资源并渲染，图标文件不存入项目文件夹。
// 原理（逆向自游戏客户端 PandaVersionManager / RES.getVirtualUrl，与遗物/魂印一致）：
//   1) 抓取版本头 https://s.61.com/zzz_config.txt 得到当前游戏版本号 ts
//   2) 抓取版本清单 https://s.61.com/version/version{ts}.swf（实为 ZIP，内含 body JSON）
//   3) 在浏览器内解压 body，读取 resource/hole/gain_icon 下每个 gain_icon_{id}.png 的 mtime
//   4) 服务器图标文件名 = CRC32("gain_icon_{id}.{mtime}.png")
//   5) 最终图标 URL = https://s.61.com/resource/hole/gain_icon/{CRC32}.png
...
(function () {
    ...
    const CACHE_PREFIX = 'seer_zengyi_icons_'
    ...
    // 对外：获取某增益 id 的图标 URL（未就绪或不存在时返回 ''）
    function getZengyiIconUrl(id) {
        return urlMap[id] || ''
    }
    // 对外：图标映射就绪后回调（已就绪则立即执行）
    function onZengyiIconsReady(fn) {
        if (ready) fn()
    ...
    function buildUrlMap(body) {
        ...
        const folder = body && body.files && body.files.resource &&
            body.files.resource.hole && body.files.resource.hole.gain_icon
        if (!folder) return map
        for (const key of Object.keys(folder)) {
            if (key.indexOf('gain_icon_') !== 0 || key.indexOf('.png') !== key.length - 4) continue
            const id = parseInt(key.slice(10, -4), 10) // 'gain_icon_' 长 10 字符
            if (!id) continue
            const v = folder[key]
            const mtime = Array.isArray(v) ? +v[0] : +v
            if (!mtime) continue
            const hash = crc32('gain_icon_' + id + '.' + mtime + '.png')
            map[id] = BASE + '/resource/hole/gain_icon/{' + hash + '}.png'
        }
        return map
    }
    ...
    async function init() {
        try {
            ...
            console.warn('[zengyi-icons] 增益图标映射加载失败，将使用品质色块兜底：', e)
        }
        waiters.splice(0).forEach(fn => fn())
    }

    window.getZengyiIconUrl = getZengyiIconUrl
    window.onZengyiIconsReady = onZengyiIconsReady
    init()
})()