/* ============ 魂印信息动态加载（启动脚本） ============ */
// 功能：每次加载项目页面时，从 s.61.com 实时抓取无光黑洞魂印配置表并渲染，数据文件不存入项目文件夹。
// 原理（逆向自游戏客户端 XlsModule + EncryptUtils，与遗物一致）：
//   1) 抓取版本头 https://s.61.com/zzz_config.txt 得到当前游戏版本号 ts
//   2) 抓取版本清单 https://s.61.com/version/version{ts}.swf（实为 ZIP，内含 body JSON），
//      读取 resource/json/xls/cn 下 equipSoul.json 的 mtime
//   3) 配置表文件名 = CRC32("equipsoul.{mtime}.json")（文件名小写），
//      最终 URL = https://s.61.com/resource/json/xls/cn/{CRC32}.json
//   4) 表内每条记录 content 以 "ELOCKEDE_" 前缀加密（AES-256-CBC / PKCS7），
//      密钥 WNOPRH3fpA2gqaYGpaNAbg4vQESrYWBU，IV ABCDEF0123456789
//   5) 解密后映射为图鉴需要的 id/名字/效果/官方描述（魂印品质固定为红色，无需筛选）
...
(function () {
    ...
    const CACHE_PREFIX = 'seer_soulseal_data_'

    // 解密参数（逆向自游戏 Common.min.js 中的 EncryptUtils.DecryptAllLineInJson("equipSoul", ...)）
    const AES_KEY = 'WNOPRH3fpA2gqaYGpaNAbg4vQESrYWBU'
    ...
    // 对外：获取动态加载的魂印数据数组（未就绪时为空数组）
    function getSoulSealData() {
        return soulList
    }
    // 对外：动态数据就绪后回调（已就绪则立即执行）
    function onSoulSealDataReady(fn) {
        if (ready) fn()
    ...
    // 从清单读取 equipSoul.json 的 mtime
    function getEquipSoulMtime(body) {
    ...
    // 解密配置表并映射为图鉴数据结构
    async function buildSoulList(arr) {
        const result = []
    ...
    async function init() {
        try {
            ...
            if (!list || !Array.isArray(list) || !list.length) {
                // 3) 下载并解压版本清单，取得 equipSoul.json 的 mtime
                const body = await loadVersionBody(ver)
                const mtime = getEquipSoulMtime(body)
                if (!mtime) throw new Error('no equipSoul mtime')

                // 4) 按哈希规则抓取配置表并解密
                const hash = crc32('equipsoul.' + mtime + '.json')
                const res = await fetch(BASE + '/resource/json/xls/cn/{' + hash + '}.json')
                if (!res.ok) throw new Error('equipSoul HTTP ' + res.status)
                const arr = JSON.parse(await res.text())
                list = await buildSoulList(arr)
                try { localStorage.setItem(cacheKey, JSON.stringify(list)) } catch (e) { /* 忽略缓存写入失败 */ }
            }
            soulList = list
            ready = true
        } catch (e) {
            console.warn('[soulseal-data] 魂印信息动态加载失败：', e)
        }
        waiters.splice(0).forEach(fn => fn())
    }

    window.getDynamicSoulSealData = getSoulSealData
    window.onSoulSealDataReady = onSoulSealDataReady
    init()
})()
